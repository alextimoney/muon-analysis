...
// calculate delta r
double  ANA_utils::delta_r(double phi1, double phi2, double eta1, double eta2) {
...
// ============================================================================================================
  ...
  // ----------------------
     ...
     // Also added leptoquarks (pdgid = 42)
 
  std::vector<int> vecboson_index;
  
  for (int i = event.size() - 1; i > 0; i--) {
    if (event[i].idAbs() == 24 || event[i].idAbs() == 23 || event[i].idAbs() == 42) {
...
//
// Find the prompt photons, fill a TruthPart object for each photon and store them in a collection. Note
// that carbon copies and chains of intermediate steps are also stored in the even record, giving many more 
// photons in the event record than there is in the event process. The last copy is the one that carry the right
// energy and momentum. To find it, the Pythia function Particle::iBotCopyID() looks for copy of the same
// particle down stream. If there is no further copy, the function returns the index of the particle tested.
// The vector bosons to keep are those for which there is no further copy in the "descendence".
//
...
// ============================================================================================================
void ANA_utils::Get_StableMuons(Pythia8::Event event, std::vector<TruthPart>* p_PromptBareLept_Coll, std::vector<float>* p_MuIso_Coll, std::vector<float>* eta_MuIso_Coll) {
//
// Find all stable muons, regardless of if they come from a weak boson or a Higgs or any exotic particle. Will
// allow to get muon from Omnia.  
//
// ============================================================================================================
 
  // Find the stable muons
  // -----------------------
 
     // Note: To get this we first look at the muons involved in the hard process and follow its carbon copy
     //       to the bottom of the chain. We verified with the daughters history that it is the right particle.
     //       The index of this particle is stored.
 
  std::vector<int> stablemuons_index;
 
 
  for (int i = 0; i < event.size(); ++i) {
    if (event[i].status() == 91 and (fabs(event[i].id()) == 13) ) {
      stablemuons_index.push_back(event[i].iBotCopy());
      float muIsoPt = 0;
      for (int j = 0; j < event.size(); j++) {
        if (event[j].isFinal() && !(fabs(event[j].idAbs()) == 13)) {
          double temp_delta_r = delta_r(event[j].phi(), event[i].phi(), event[j].eta(), event[i].eta());
          if (temp_delta_r < .1) {
            muIsoPt += event[j].pT();
          }
        }
      }
      p_MuIso_Coll->push_back(muIsoPt); 
      eta_MuIso_Coll->push_back(event[i].eta());
    }
  }
  ...
  if (stablemuons_index.size() == 0) return;
 
  for (int i_mu = 0; i_mu < stablemuons_index.size(); i_mu++)
    {
      ...
      Fill_TruthPart(event, stablemuons_index[i_mu], p_temp_mu);
     
    
      // Store the truth particle the vector boson collection
      // ....................................................
 
      p_PromptBareLept_Coll->push_back(temp_mu);

      // float muIsoPt = 0;
      //   for (int j = 0; j < event.size(); j++) {
      //     if (event[j].isFinal() && !(fabs(event[j].idAbs()) == 13)) {
      //       double temp_delta_r = delta_r(event[j].phi(), event[i_mu].phi(), event[j].eta(), event[i_mu].eta());
      //       if (temp_delta_r < .1) {
      //         muIsoPt += event[j].pT();
      //       }
      //     }
      //   }
      //   p_MuIso_Coll->push_back(muIsoPt); 
      //   eta_MuIso_Coll->push_back(event[i_mu].eta());
    }
}
// ============================================================================================================
...
void ANA_utils::Get_MuonIsolation(Pythia8::Event event, std::vector<float>* p_MuIso_Coll, std::vector<float>* eta_MuIso_Coll) {
  int nMuons = 0;

  // if ((event[i].status() == 91 && (fabs(event[i].id()) == 13)) || (event[i].status() == 91 && (fabs(event[i].id()) == -13))) {
  //   nMuons ++;
  // }
  ...
  for(int i = 0; i < nMuons; i++) {
    float muIsoPt = 0;
    for (int j = 0; j < event.size(); j++) {
      if (event[j].isFinal() && !(fabs(event[j].id()) == 13)) {
        // && !(fabs(event[j].id()) == -13)
        double temp_delta_r = delta_r(event[j].phi(), event[i].phi(), event[j].eta(), event[j].phi());
        if (temp_delta_r < .1) {
          muIsoPt += event[j].pT();
        }
      }
...
// ============================================================================================================
void ANA_utils::Get_BarePromptLepton(Pythia8::Event event, std::vector<int> vecboson_index, std::vector<TruthPart>* p_PromptBareLept_Coll, std::vector<TruthPart>* p_ConversionLept_Coll, std::vector<TruthPart>* p_PhotonFSR_Coll, std::vector<TruthPart>* p_Neutrino_Coll, std::vector<float>* p_MuIso_Coll, std::vector<float>* eta_MuIso_Coll) {
//
...
// ============================================================================================================
  ...
  std::vector<int> PromptLepton_index;
  std::vector<int> ConversionLepton_index;
  ...
  for (int i=0; i< vecboson_index.size(); i++) {
      ...
      if (gen2.size() != 0) {
        ...
        if (gen3.size() != 0) {
	      ...
	      for (int i_gen5 = 0; i_gen5 < gen5.size(); i_gen5++) {
              ...
              // Check if the particles in the sevenht generation are stable, if not moved to the next
              // ....................................................................................  

		if (gen7.size() != 0) {
		  for (int i_gen7 = 0; i_gen7 < gen7.size(); i_gen7++) {
		    if ( event[gen7[i_gen7]].isFinal()  ) StableDaughterList.push_back(gen7[i_gen7]);
		    else {
		      for (int d8 = 0; d8 <  (event[gen7[i_gen7]].daughterList()).size(); d8++) {
			gen8.push_back((event[gen7[i_gen7]].daughterList())[d8]);
		      }
		    }
		  }
    ...
    for (int stab_i = 0; stab_i < StableDaughterList.size(); stab_i++) {
      ...
      if ( event[temp_id].idAbs() == 11 || event[temp_id].idAbs() == 13 )
      {
        int mother = event[temp_id].mother1();
        if ( (event[mother]).idAbs() == 22 ) {
          ConversionLepton_index.push_back(StableDaughterList[stab_i]);
        } 
        else {
          PromptLepton_index.push_back(StableDaughterList[stab_i]);

          float muIsoPt = 0;
          if(event[temp_id].idAbs() == 13) {
            for (int j = 0; j < event.size(); j++) {
              if (event[j].isFinal() && !(fabs(event[j].idAbs()) == 13)) {
                // && !(fabs(event[j].id()) == -13)
                double temp_delta_r = delta_r(event[j].phi(), event[temp_id].phi(), event[j].eta(), event[temp_id].eta());
                if (temp_delta_r < .1) {
                  muIsoPt += event[j].pT();
                }
              }
            ...
            p_MuIso_Coll->push_back(muIsoPt); 
            eta_MuIso_Coll->push_back(event[temp_id].eta());
          }
        }
      } 
  ...
  if (PromptLepton_index.size() != 0) {
    for (int i_lep = 0; i_lep < PromptLepton_index.size(); i_lep++) {
      ...
      Fill_TruthPart(event, PromptLepton_index[i_lep], p_temp_lep);
      ...
      // float muIsoPt = 0;
      // if(PromptLepton_index[i_lep].Pdgid() == 13) {
      //   for (int j = 0; j < event.size(); j++) {
      //     if (event[j].isFinal() && !(fabs(event[j].idAbs()) == 13)) {
      //       // && !(fabs(event[j].id()) == -13)
      //       double temp_delta_r = delta_r(event[j].phi(), event[i_lep].phi(), event[j].eta(), event[i_lep].eta());
      //       if (temp_delta_r < 10) {
      //         muIsoPt += event[j].pT();
      //       }
      //     }
      ...
      // }
      // p_MuIso_Coll->push_back(muIsoPt); 
      // eta_MuIso_Coll->push_back(PromptLepton_index[i_lep].eta()); 
    }
  }

    if (ConversionLepton_index.size() != 0) {
    for (int i_lepc = 0; i_lepc < ConversionLepton_index.size(); i_lepc++) {

      TruthPart temp_lepc;
      TruthPart* p_temp_lepc;

      p_temp_lepc = &temp_lepc;

      Fill_TruthPart(event, ConversionLepton_index[i_lepc], p_temp_lepc);
    
      // Store the 4 vector in the bare electron collection
      // ..................................................

      p_ConversionLept_Coll->push_back(temp_lepc);
  ...
  for (int i=0; i< vecbosindex.size(); i++)
    {
      ...
      int pdgidboson = event[idxVb].idAbs();
