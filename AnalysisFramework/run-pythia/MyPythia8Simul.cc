...
void MyAnalysis::init()
{
  ...
  tree->Branch("muonisolation_pt", &muonisolation_pt);
  tree->Branch("muonisolation_eta", &muonisolation_eta);
  tree->Branch("muonisolation_phi", &muonisolation_phi);
  tree->Branch("muonisolation_E", &muonisolation_E);
...
void MyAnalysis::analyze(Event& event, Event& partonevent, std::vector<double> EventWeights)
{
  ...
  p_MuIso_Coll = &MuIso_Coll;
  eta_MuIso_Coll = &MuIso_Coll_Eta;
  ...
  // Get Muon Isolation
  // .................
  // myUtils.Get_MuonIsolation(event, p_MuIso_Coll, eta_MuIso_Coll);
  ...
  {
    myUtils.Get_BarePromptLepton(event, vecbosindex, p_PromptLeptonBare_Coll, p_LeptonConversion_Coll, p_FSRPhoton_Coll, p_Neutrino_Coll, p_MuIso_Coll, eta_MuIso_Coll);
  ...
  if(vecbosindex.size() == 0) {
    myUtils.Get_StableMuons(event, p_PromptLeptonBare_Coll, p_MuIso_Coll, eta_MuIso_Coll);
  }
  ...
  // Muon Isolation
  // ..............
  if(p_MuIso_Coll->size() != 0) {
    for(size_t i = 0; i < p_MuIso_Coll->size(); i++) {
      muonisolation_pt.push_back(MuIso_Coll[i]);
      muonisolation_eta.push_back(MuIso_Coll_Eta[i]);
    }
  }
  ...
  muonisolation_pt.clear();
  muonisolation_eta.clear();
  muonisolation_phi.clear();
  muonisolation_E.clear();
