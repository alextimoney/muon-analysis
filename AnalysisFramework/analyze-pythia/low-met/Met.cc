...

{
  ...
  leg->AddEntry(met1, "0 < MET [GeV] < 5","f");
  leg->AddEntry(met2, "5 < MET [GeV] < 10", "f");
  leg->AddEntry(met3, "10 < MET [GeV] < 20", "f");
  leg->AddEntry(met4, "20 < MET [GeV] < 30", "f");
  leg->AddEntry(met5, "30 < MET [GeV]", "f");
  // leg->AddEntry(met6, "40 < MET [GeV] < 60", "f");
  // leg->AddEntry(met7, "60 < MET [GeV] < 80", "f");
  // leg->AddEntry(met8, "80 < MET [GeV]", "f");
  leg->SetTextSize(0.035);
  leg->Draw();

  cx->SetLogy();
  cx->Print("./plots/muon-pt/met.png");
  cx->Close();
