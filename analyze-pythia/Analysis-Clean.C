...
TH1F* BareMuon_pT_raw = new TH1F("BareMuon_pT_raw", "Momentum of muons without normalization", num_bins, 0, 120);
TH1F* Muon_P = new TH1F("Muon_P", "Magnitude of muon momentum with 1 < |eta| < 2.5 normalized to cross-section", num_bins, 0, 120);
TH1F* Muon_P_raw = new TH1F("Muon_P_raw", "Magnitude of muon momentum with 1 < |eta| < 2.5 (raw data)", 120, 0, 120);
...
int RunAnalysis(string sample)
{
    ...
    while(treeReader.Next())
    {
        ...
        for(int i = 0; i < MuonTrueContainer.size(); i++) {
            ...
            BareMuon_pT_raw->Fill(MuonTrueContainer[i].Pt(), wgt);
            Muon_P_raw->Fill(MuonTrueContainer[i].Pt(), wgt);
        }
